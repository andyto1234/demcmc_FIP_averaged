...
class asheis:
    ...
    '''
    def __init__(self, filename, ncpu='max', rebin=False, eiscube=None):
        self.filename = filename
        self.ncpu = ncpu
        self.rebin = rebin
        self.eiscube = eiscube
    ...
    def fit_data(self, line, product, refit, outdir):
        ...
        template, template_name = self.read_template(line) # Read template

        path = Path(f'{self.filename}'.replace("data.h5", template_name).replace(".template", f"-{self.dict[f'{line}'][1]}.fit"))
        
        if path.is_file() == False or refit == True or self.rebin != False or self.eiscube is not None:
            if self.eiscube is not None:
                cube = self.eiscube
            else:
                cube = eispac.read_cube(self.filename, window=template.central_wave)
            ...
            if self.eiscube is None:  # Only save fit results if working with a file
    ...
    def fit_cube(self, line):
        # for file in eis_evts:
        template, template_name = self.read_template(line)
    ...
    def read_density_file(self, line):
        ...
        return density_ratios, density_values

    def find_closest_density(self, obs, density_ratios, density_values):
        closest_index = np.argmin(np.abs(density_ratios - obs))
        return density_values[closest_index]
    ...
    def get_density(self, outdir='./', refit=False, plot=True, mcmc=False, **kwargs):
        ...
        density_ratios, density_values = self.read_density_file('fe_13_203.83')
        ...
        for i in range(obs_ratio.shape[0]):
            for j in range(obs_ratio.shape[1]):
                obs = obs_ratio[i, j]
                obs_ratio[i, j] = self.find_closest_density(obs, density_ratios, density_values)
